...
export class FireService {
  ...
  async getFireBalance(workerProfileId: string): Promise<WorkerFires | null> {
    ...
    if (error && error.code !== 'PGRST116') {
      ...
      throw new FireError('Failed to fetch Fire balance', FireErrorCodes.DATABASE_ERROR);
    }
  ...
  async initializeFireBalance(workerProfileId: string): Promise<WorkerFires> {
    ...
    if (error) {
      throw new FireError('Failed to initialize Fire balance', FireErrorCodes.DATABASE_ERROR);
    }
  ...
  async validatePurchase(
    ...
    if (wallet.balance_usd < costUSD) {
      return {
        valid: false,
        error: `Insufficient wallet balance. Need $${costUSD.toFixed(2)}, have $${wallet.balance_usd.toFixed(2)}`,
      };
    }
  ...
  async purchaseFire(
    ...
    userId: string,
    fireAmount: number
  ): Promise<{ fire: WorkerFires; transaction: FireTransaction; walletTransactionId: string }> {
    // Validate purchase
    const validation = await this.validatePurchase(workerProfileId, userId, fireAmount);
    if (!validation.valid) {
      throw new FireError(validation.error!, FireErrorCodes.INVALID_AMOUNT, 400);
    }
    ...
    try {
      ...
      if (!wallet) {
        throw new FireError('Wallet not found', FireErrorCodes.WORKER_NOT_FOUND);
      }
      ...
      const { data: walletTx } = await this.supabase
        .from('transactions')
        .insert({
          user_id: userId,
          wallet_id: wallet.id,
      ...
      const { data: fireTx } = await this.supabase
        ...
        .insert({
          ...
          description: `Purchased ${fireAmount} Fire for $${costUSD.toFixed(2)}`,
  ...
  async hasClaimedToday(workerProfileId: string): Promise<boolean> {
    ...
    if (error && error.code !== 'PGRST116') {
      throw new FireError('Failed to check daily login', FireErrorCodes.DATABASE_ERROR);
    }
  ...
  async claimDailyLogin(workerProfileId: string): Promise<{
    ...
    try {
      ...
      const { data: updatedFire } = await this.supabase
        ...
        .update({
          total_fires: newFireBalance,
          lifetime_fires_earned: fireBalance.lifetime_fires_earned + rewardAmount,
        })
        .eq('worker_profile_id', workerProfileId)
        .select()
        .single();
  ...
  async getActiveBoosts(workerProfileId: string): Promise<ActiveBoostInfo[]> {
    ...
    if (error) {
      throw new FireError('Failed to fetch active boosts', FireErrorCodes.DATABASE_ERROR);
    }
   ...
   */
  async hasActiveBoost(workerProfileId: string, boostType: FireBoostType): Promise<boolean> {
  ...
  async activateBoost(
    workerProfileId: string,
    boostType: FireBoostType
  ): Promise<{ boost: FireBoost; fire: WorkerFires; transaction: FireTransaction }> {
    const config = FIRE_BOOST_CONFIG[boostType];

    // Check status
    const status = await this.checkBoostStatus(workerProfileId, boostType);
    if (!status.can_activate) {
      throw new FireError(status.reason!, FireErrorCodes.INSUFFICIENT_FIRE, 400);
    }
    ...
    try {
      ...
      if (status.has_active_boost) {
        ...
        // New boost
        expiresAt = new Date(now.getTime() + config.durationHours * 60 * 60 * 1000);
      }
  ...
  async expireBoosts(): Promise<number> {
    ...
    if (error) {
      throw new FireError('Failed to expire boosts', FireErrorCodes.DATABASE_ERROR);
    }
  ...
  async getTransactions(
    ...
    if (error) {
      throw new FireError('Failed to fetch transactions', FireErrorCodes.DATABASE_ERROR);
    }
